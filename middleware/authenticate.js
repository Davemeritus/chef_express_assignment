  ...
  try {
    ...
    if (!req.headers.username || !req.headers.password) {
      return res.status(401).json({
        sucess: false,
        message: "required login details not provided",
      });
    }
    ...
    if (
      ...
      authorizedUsers.regularUser.some(
        (userInfo) =>
          userInfo.username === username && userInfo.password === password,
      )
    ) {
      ...
      res
        .status(401)
        .json({ sucess: false, message: "wrong username or password" });
    }
  } catch (err) {
